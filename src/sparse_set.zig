...
test "SparseSet iteration after remove is correct" {
    const allocator = std.testing.allocator;
...
test "SparseSet with different key types" {
    const allocator = std.testing.allocator;

    // Test with u8 keys (256 max keys)
    var set_u8 = try SparseSet(u8, []const u8).init(allocator, 256, 4);
    defer set_u8.deinit();

    try set_u8.put(0, "zero");
    try set_u8.put(255, "max");
    try std.testing.expectEqualStrings("zero", set_u8.get(0).?);
    try std.testing.expectEqualStrings("max", set_u8.get(255).?);

    // Test with u16 keys
    var set_u16 = try SparseSet(u16, f32).init(allocator, 1000, 4);
    defer set_u16.deinit();

    try set_u16.put(500, 3.14);
    try std.testing.expectEqual(@as(?f32, 3.14), set_u16.get(500));
}
