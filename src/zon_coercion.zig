...
test "coerceValue: simple struct with all fields" {
    ...
    const result = coerceValue(Target, .{ .x = 10, .y = 20 });
...
test "coerceValue: nested struct" {
    ...
    const result = coerceValue(Outer, .{ .inner = .{ .value = 42 }, .name = "test" });
    try std.testing.expectEqual(@as(i32, 42), result.inner.value);
}

test "coerceValue: union with enum literal" {
    const State = union(enum) { idle, running: u32 };
    const result = coerceValue(State, .idle);
    try std.testing.expect(result == .idle);
}

test "coerceValue: union with payload" {
    const Shape = union(enum) { circle: struct { radius: f32 }, rect: struct { w: f32, h: f32 } };
    const result = coerceValue(Shape, .{ .circle = .{ .radius = 5.0 } });
    try std.testing.expectEqual(@as(f32, 5.0), result.circle.radius);
}

test "tupleToSlice: converts tuple to slice" {
    const slice = tupleToSlice(i32, .{ 1, 2, 3 });
...
test "buildStruct: with defaults" {
    const Config = struct { width: i32 = 800, height: i32 = 600, title: []const u8 };
    const result = buildStruct(Config, .{ .title = "Test" });
    try std.testing.expectEqual(@as(i32, 800), result.width);
    try std.testing.expectEqual(@as(i32, 600), result.height);
}

test "mergeStructs: override values" {
    const base = .{ .x = 10, .y = 20, .color = "red" };
    const overrides = .{ .color = "blue" };
    const result = mergeStructs(base, overrides);
    try std.testing.expectEqual(@as(i32, 10), result.x);
    try std.testing.expectEqualStrings("blue", result.color);
}
